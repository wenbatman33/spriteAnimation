...
const PERIOD = 5.4;
const asset=name=>new URL(`../assets/western/${name}.webp`,import.meta.url).href;
...
export function createWestern(host, gunnerSource = asset('gunner')) {
  host.innerHTML = `<div class="western-art"><img class="western-backdrop" src="${asset('background')}" alt="夕陽西部城鎮與黃金老虎機"><div class="western-reels" aria-label="五列動畫轉輪">${Array.from({length:5},(_,column)=>`<div class="western-reel"><div class="western-strip">${Array.from({length:12},(_,i)=>`<div class="western-symbol"><div class="symbol-crop"><img src="${asset('symbols')}" alt="${['警徽','牛仔帽','左輪','炸藥','馬蹄鐵','寶石'][i%6]}" style="top:${-(i%6)*100}%"></div></div>`).join('')}</div></div>`).join('')}</div><div class="western-gunner"><img src="${gunnerSource}" alt="牛仔猴子開槍動畫" draggable="false"></div></div><div class="western-controls"><label><input type="checkbox" data-shoot checked>連續開槍</label><label><input type="checkbox" data-reels checked>循環轉輪</label><button type="button" data-spin>轉動一次</button><span class="western-state">轉輪準備中</span></div>`;
