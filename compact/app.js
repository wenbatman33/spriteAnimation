...
for(const [index,ad]of ads.entries()){
 const card=document.createElement('article');card.className='card';card.id=ad.id;card.innerHTML='<div class="card-head"><h2></h2><span class="kind">365 × 160</span></div><div class="stage-wrap"><div class="banner-surface"></div></div><p class="status" role="status">捲動至此載入</p><div class="card-body"><div class="tabs" role="group" aria-label="切換格式"></div><p class="facts"></p><div class="actions"><button class="quiet pick" aria-pressed="false">選擇此格式</button><button class="quiet deliver">下載／嵌入</button></div></div>';
