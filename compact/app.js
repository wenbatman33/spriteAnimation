...
for(const [index,ad]of ads.entries()){
 ...
 card.querySelector('.kind').textContent=`${ad.width||365} × ${ad.height||160}`;card.querySelector('h2').textContent=`0${index+1}  ${ad.name}`;const item={ad,card,host:card.querySelector('.banner-surface'),status:card.querySelector('.status'),facts:card.querySelector('.facts'),pick:card.querySelector('.pick'),format:initial,token:0,controller:null,visible:false};cards.push(item);
...
function updateCode(){if(!selection)return;const{ad,format}=selection,url=safeDestination($('#url').value);let code;
 if(['js','spine'].includes(format)){const src=new URL('./play.html',import.meta.url);src.searchParams.set('id',ad.id);src.searchParams.set('format',format);if(url)src.searchParams.set('url',url);code=`<iframe src="${escape(src.href)}" title="${ad.name}" width="${ad.width||365}" height="${ad.height||160}" style="border:0;display:block" loading="lazy"></iframe>`;
